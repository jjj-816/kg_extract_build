...
| 03 | 结果类型筛选、文档/规范/图线索证据区分及降级提示 | `dashboard_audit.py` 的结果详情与 `split_evidence()`；`test_dashboard_audit_report_ui.py::test_result_detail_labels_three_evidence_kinds`；全量回归 | 通过 |
| 04 | 人工复核、发布门禁、发布后 JSON/DOCX 下载 | `test_dashboard_audit_report_ui.py`：门禁、发布、两个下载控件；`report_service.py` | 通过 |
| 05 | 历史版本、人工更正、前序关联、重新审核门禁 | `dashboard_audit.py` 的历史/更正入口；`test_dashboard_audit_report_ui.py::test_history_lookup_and_correction_form`；`report_service.py`、`ui_state.py` | 通过 |
