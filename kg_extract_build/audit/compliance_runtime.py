...
class ComplianceRuntime:
    ...
    def run(self, task, evidence, *, scope_preflight: NormativeScopePreflight, run_id: str) -> TaskExecutionResult:
        ...
        if plan:
            ...
            record("retrieval_planning", "completed" if not plan.diagnostics else "degraded", {"evidence_block_ids": list(getattr(plan, "document_block_ids", ()))}, {"normative_queries": list(plan.normative_queries), "graph_queries": list(getattr(plan, "graph_queries", ())), "diagnostics": list(plan.diagnostics), "llm_prompt": planner_io.get("prompt") if planner_io else None, "llm_messages": planner_io.get("messages") if planner_io else None, "llm_raw_response": planner_io.get("raw_response") if planner_io else None, "llm_parsed_response": planner_io.get("parsed_response") if planner_io else None})
