...
class ComplianceRuntime:
    def __init__(self, normative_search: Callable[..., Mapping[str, Any]], model: Callable[..., Mapping[str, Any]]):
    ...
    def run(self, task, evidence, *, scope_preflight: NormativeScopePreflight, run_id: str) -> TaskExecutionResult:
        ...
        search_result = self.normative_search(query="\n".join(item.get("raw_text", "") for item in evidence), scope=scope_preflight.scope)
        ...
        return TaskExecutionResult(task.task_id, task.route, "completed", output["result_status"], document_evidence + normative_evidence, issues=issues, diagnostics=(f"normative_clause_count={len(normative_evidence)}",))
