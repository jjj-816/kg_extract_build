...
class ReasonablenessRuntime:
    ...
    def run(self, task, evidence, graph_result: GraphRetrievalResult, run_id: str, normative_search=None, retrieval_plan=None, planner_interaction=None):
        ...
        if retrieval_plan is not None:
            trace.append({"step": 2, "stage": "retrieval_planning", "status": "completed" if not retrieval_plan.diagnostics else "degraded", "input": {"prompt_version": retrieval_plan.prompt_version, "evidence_block_ids": list(retrieval_plan.document_block_ids)}, "output": {"normative_queries": list(retrieval_plan.normative_queries), "graph_queries": list(retrieval_plan.graph_queries), "relationship_types": list(retrieval_plan.relationship_types), "diagnostics": list(retrieval_plan.diagnostics), "llm_prompt": planner_interaction.get("prompt") if planner_interaction else None, "llm_messages": planner_interaction.get("messages") if planner_interaction else None, "llm_raw_response": planner_interaction.get("raw_response") if planner_interaction else None, "llm_parsed_response": planner_interaction.get("parsed_response") if planner_interaction else None, "correction_raw_response": planner_interaction.get("correction_raw_response") if planner_interaction else None, "correction_parsed_response": planner_interaction.get("correction_parsed_response") if planner_interaction else None}, "error": None})
        evidence.extend({
